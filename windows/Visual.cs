...
public class DarkComboBox:ComboBox {
 ...
 protected override void WndProc(ref Message m){base.WndProc(ref m);if(m.Msg==0xF&&IsHandleCreated){using(var g=Graphics.FromHwnd(Handle)){var r=new Rectangle(Math.Max(0,Width-24),1,23,Height-2);using(var b=new SolidBrush(MainForm.PanelColor))g.FillRectangle(b,r);using(var pen=new Pen(Color.FromArgb(58,62,77)))g.DrawRectangle(pen,0,0,Width-1,Height-1);g.SmoothingMode=SmoothingMode.AntiAlias;using(var pen=new Pen(Enabled?Color.White:MainForm.Muted,1.4f)){float x=Width-13,y=Height/2f;g.DrawLines(pen,new[]{new PointF(x-4,y-2),new PointF(x,y+2),new PointF(x+4,y-2)});}}}}
...
public class DarkMenuRenderer:ToolStripProfessionalRenderer {
 ...
 protected override void OnRenderToolStripBackground(ToolStripRenderEventArgs e){e.Graphics.Clear(MainForm.PanelColor);var r=new RectangleF(0,0,e.ToolStrip.Width-1,e.ToolStrip.Height-1);e.Graphics.SmoothingMode=SmoothingMode.AntiAlias;Visual.Fill(e.Graphics,r,10,Color.FromArgb(26,29,40));Visual.Stroke(e.Graphics,r,10,Color.FromArgb(58,62,77));}
...
public static class DarkMenu {
 ...
 public static ContextMenuStrip Create(){return new ContextMenuStrip{MaximumSize=new Size(680,0),BackColor=Color.FromArgb(26,29,40),ForeColor=Color.FromArgb(230,232,240),Font=Visual.Font(14),ShowImageMargin=true,Renderer=renderer,Padding=new Padding(0,4,0,4)};}
}
}
