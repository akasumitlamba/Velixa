...
public partial class MainForm {
 ...
 void BuildDesk(){
  ...
  var hotkeys=new NavigationButton("Help","info",false);hotkeys.Click+=(s,e)=>ShowSection("Help");nav.Controls.Add(hotkeys);
  ...
  var input=InspectorPanel(right,"Input sharing","bluetooth");input.Visible=false;var device=InspectorPanel(right,"This desk","monitor");var screen=InspectorPanel(right,"Screen layout","devices");
  ...
  LabelAt(device,"Selected device",18,60,180,22,14,Muted);var nameBox=new SoftPanel{Radius=9,FillColor=Color.FromArgb(25,33,47)};device.Controls.Add(nameBox);selectedName=LabelAt(nameBox,"",10,11,180,24,13,Color.White);selectedName.AutoEllipsis=true;
  LabelAt(device,"Keyboard & mouse source",18,135,190,22,14,Muted);sourceButton=Button("Automatic (recommended)",SourceMenu);device.Controls.Add(sourceButton);sourceName=LabelAt(device,"",18,204,220,22,12,Muted);sourceName.AutoEllipsis=true;
  ...
  pauseButton=Button("Pause sharing",ToggleInput);nav.Controls.Add(pauseButton);retryButton=Button("Retry connection",RetryDesk);nav.Controls.Add(retryButton);disconnectButton=Button("Disconnect",()=>{if(ConfirmAction("Disconnect?","Stop sharing on this PC and return to setup?","Disconnect")){Stop();Wire.SaveSecret("role.bin","");Welcome();}});nav.Controls.Add(disconnectButton);deskCount=new Label();
  Action layout=()=>{
   ...
   int panelW=rightW-(root.Height<504?SystemInformation.VerticalScrollBarWidth:0);int y=14;foreach(var b in new[]{home,devices,settings,hotkeys}){b.SetBounds(12,y,navW-24,44);y+=51;}slogan.Visible=false;
   ...
   device.SetBounds(0,16,panelW,292);screen.SetBounds(0,326,panelW,112);
   ...
   int bottom=Math.Max(456,right.Height-48);pauseButton.SetBounds(12,352,navW-24,40);retryButton.SetBounds(12,400,navW-24,40);disconnectButton.SetBounds(12,448,navW-24,40);micButton.SetBounds(12,232,navW-24,36);micLabel.SetBounds(18,275,navW-36,60);
   right.AutoScrollMinSize=new Size(0,504);right.AutoScroll=true;

  };root.Resize+=(s,e)=>layout();layout();sectionHost=root;deskCenter=center;deskInspector=right;navItems=new[]{home,devices,settings,hotkeys};sectionPage=new ScrollSurface{Visible=false,AutoScroll=true,FillColor=PanelColor};root.Controls.Add(sectionPage);root.Resize+=(s,e)=>LayoutSection();UpdateDesk();
 }
 Label micLabel;
 ...
 void RefreshInspector(){if(session==null||selectedName==null||selectedName.IsDisposed)return;var selected=desk.Selected;if(selected!=null&&!session.Devices.Contains(selected))desk.Selected=selected=null;var actions=deskInspector.Controls.Cast<Control>().SelectMany(c=>c.Controls.Cast<Control>()).FirstOrDefault(c=>c.Name=="DeviceActions");if(actions!=null)actions.Enabled=selected!=null;sourceButton.Enabled=deskButton.Enabled=session.Connected;selectedName.Text=selected==null?"Select a device":selected.name+(selected.id==Network.LocalId?" · This PC":"");sourceButton.Text=session.Automatic?(sourceButton.Width<210?"Automatic":"Automatic (recommended)"):"Choose input source";deviceBadge.Text=session.Devices.Count.ToString();inputToggle.Checked=!paused;inputToggle.Invalidate();clipboardToggle.Enabled=sharing!=null;clipboardToggle.Checked=sharing!=null&&sharing.ClipboardEnabled;clipboardToggle.Invalidate();connectionLabel.Text=session.Connected?(session.Devices.Any(d=>d.id!=Network.LocalId&&d.Available)?"Connected":"No peers online"):network!=null&&network.Connecting?"Connecting":"Offline";retryButton.Visible=network!=null&&!network.IsHost&&!session.Connected;retryButton.Enabled=network!=null&&!network.Connecting;connectionLabel.Parent.Invalidate();int online=session.Devices.Count(d=>d.id!=Network.LocalId&&d.Available);status.Text=session.Connected?(online+" device"+(online==1?"":"s")+" online · Ctrl + Alt + Backspace to return"):network!=null&&network.Connecting?"Connecting · up to 3 attempts":"Desk offline · turn on the other PC, then Retry";pauseButton.Visible=true;pauseButton.Enabled=session.Connected;}
}
