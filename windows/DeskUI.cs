...
public partial class MainForm {
 ...
 void BuildChrome(){
  ...
  var settings=Button("",()=>{Settings();RefreshInspector();});settings.Name="Settings";settings.Width=40;settings.IconName="settings";settings.Paint+=(s,e)=>UiIcons.Draw(e.Graphics,"settings",new RectangleF(10,10,20,20),Muted);brand.Controls.Add(settings);
  var more=Button("",()=>{var menu=DarkMenu.Create();menu.Items.Add("Microphone sharing…",null,(s,e)=>MicrophoneDialog());menu.Items.Add("Quit Velixa",null,(s,e)=>{closing=true;Close();});menu.Show(brand,new Point(brand.Width-200,64));});more.IconName="more";more.Paint+=(s,e)=>UiIcons.Draw(e.Graphics,"more",new RectangleF(10,10,20,20),Muted);brand.Controls.Add(more);
 ...
 void BuildDesk(){
  ...
  var home=new NavigationButton("My Desk","monitor",true);home.Click+=(s,e)=>{desk.Focus();desk.Fit();};nav.Controls.Add(home);
  var devices=new NavigationButton("Devices","devices",false);devices.Click+=(s,e)=>{var menu=DarkMenu.Create();foreach(var d in session.Devices){var selected=d;menu.Items.Add(d.name,null,(ss,ee)=>{desk.Selected=selected;RefreshInspector();desk.Invalidate();});}menu.Show(devices,0,devices.Height);};nav.Controls.Add(devices);
  var settings=new NavigationButton("Settings","settings",false);settings.Click+=(s,e)=>{Settings();RefreshInspector();};nav.Controls.Add(settings);
  var hotkeys=new NavigationButton("Hotkeys","grid",false);hotkeys.Click+=(s,e)=>Info("Hotkeys","Ctrl + Alt + Backspace\nReturn control to this PC from another screen.");nav.Controls.Add(hotkeys);
  var about=new NavigationButton("About","info",false);about.Click+=(s,e)=>Info("Velixa","One input. Everywhere.\nConnect your screens securely on your local network.");nav.Controls.Add(about);
  deviceBadge=LabelAt(devices,"",devices.Width-40,13,28,22,12,Color.White,true);deviceBadge.TextAlign=ContentAlignment.MiddleCenter;deviceBadge.BackColor=Color.FromArgb(38,47,67);deviceBadge.Anchor=AnchorStyles.Top|AnchorStyles.Right;
  ...
  var add=new DeskActionButton{Text="Add device",Primary=true};add.Click+=(s,e)=>PairDialog();add.Name="AddDevice";add.Visible=network!=null&&network.IsHost;center.Controls.Add(add);deskButton=new DeskActionButton{Text="Arrange"};deskButton.Click+=(s,e)=>DesksMenu();center.Controls.Add(deskButton);
  ...
  IconLabel(input,"Keyboard & mouse","keyboard",64);IconLabel(input,"Share clipboard","clipboard",112);
  inputToggle=new ToggleSwitch{Checked=!paused,AccessibleName="Keyboard and mouse sharing"};inputToggle.Click+=(s,e)=>ToggleInput();input.Controls.Add(inputToggle);
  ...
  var options=Button("More options   ›",()=>desk.ShowSelectedOptions());device.Controls.Add(options);
  LabelAt(screen,"Fit to arrangement",18,62,170,25,13,Color.White);var fit=Button("Fit",()=>desk.Fit());screen.Controls.Add(fit);
  micButton=Button("Microphone",()=>MicMenu(micLabel));nav.Controls.Add(micButton);micButton.AccessibleName="Choose microphone";
  micLabel=LabelAt(nav,MicName(),8,0,210,22,12,Muted);micLabel.AutoEllipsis=true;
  ...
  };root.Resize+=(s,e)=>layout();layout();UpdateDesk();
 }
 Label micLabel;
...
public static class UiIcons {
 ...
 static void Logo(Graphics g){
  using(var path=new GraphicsPath()){path.AddBezier(1,7,1,5,2,5,3,5.5f);path.AddLine(3,5.5f,9,8);path.AddBezier(9,8,10,8.4f,10,9,10,10);path.AddLine(10,10,10,21);path.AddBezier(10,21,10,22,9,22,8,21.6f);path.AddLine(8,21.6f,2,19);path.AddBezier(2,19,1,18.6f,1,18,1,17);path.CloseFigure();using(var b=new LinearGradientBrush(new Point(1,5),new Point(10,22),Color.FromArgb(40,201,255),Color.FromArgb(125,91,255)))g.FillPath(b,path);}
  using(var path=new GraphicsPath()){path.AddBezier(12,5,12,4,12,3.6f,13,3.2f);path.AddLine(13,3.2f,18,1);path.AddBezier(18,1,19,0.5f,20,1,20,2);path.AddLine(20,2,20,9);path.AddBezier(20,9,20,10,19.5f,10.5f,19,10.7f);path.AddLine(19,10.7f,13,13);path.AddBezier(13,13,12,13.4f,12,12.7f,12,12);path.CloseFigure();using(var b=new LinearGradientBrush(new Point(12,1),new Point(20,13),Color.FromArgb(49,191,255),Color.FromArgb(112,94,252)))g.FillPath(b,path);}
  using(var path=new GraphicsPath()){path.AddBezier(12,17,12,15,13,14,15,14);path.AddLine(15,14,20,15);path.AddBezier(20,15,22,15.4f,23,16.3f,23,18);path.AddLine(23,18,23,21);path.AddBezier(23,21,23,23,21,24,19,23.5f);path.AddLine(19,23.5f,14,22);path.AddBezier(14,22,12.5f,21.6f,12,21,12,19);path.CloseFigure();using(var b=new LinearGradientBrush(new Point(12,14),new Point(23,24),Color.FromArgb(115,117,255),Color.FromArgb(174,69,245)))g.FillPath(b,path);}
 }
}
