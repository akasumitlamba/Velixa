...
public partial class MainForm:Form {
 ...
 void Attach(){
  ...
  microphone.ReceiveEnabled=Wire.LoadSecret("mic-receive.bin","false")=="true";
  ...
  microphone.Status=message=>{if(micLabel!=null&&!micLabel.IsDisposed)micLabel.Text=message;tray.Text=(message=="Microphone sharing is on"||message=="Receiving shared microphone")?"Velixa · Microphone sharing":"Velixa";UpdateDesk();status.Text=message;};
 ...
 void MicMenu(Label label){
  ...
  bool canSend=microphone!=null&&session!=null&&session.Connected&&microphone.Targets().Length>0;
  if(!canSend)menu.Items.Add("Connect a compatible Windows PC to send audio").Enabled=false;
  ...
  foreach(var m in localMics){
   ...
   item.Enabled=canSend;item.Click+=(s,e)=>{
    ...
    StartMicToPeers(id);label.Text=microphone.State;
   };
   menu.Items.Add(item);
  }
  if(microphone!=null&&session!=null){
   foreach(var kv in microphone.RemoteMics.Where(k=>session.Devices.Any(d=>d.id==k.Key&&d.Available))){
    ...
    foreach(var rm in kv.Value){
     ...
     rItem.Enabled=microphone.ReceiveEnabled;rItem.Click+=(s,e)=>{
      ...
      microphone.RequestRemoteMic(peerId,rDevId);label.Text=microphone.State;
     };
     menu.Items.Add(rItem);
    }
   }
  }
  menu.Items.Add(new ToolStripSeparator());menu.Items.Add("Configure microphone…",null,(s,e)=>MicrophoneDialog()).Enabled=microphone!=null;menu.Show(micButton,0,micButton.Height+6);
 }
 void StartMicToPeers(int devId){
  if(microphone==null||session==null)return;
  microphone.StopReceiving();
  microphone.StartMany(microphone.Targets(),devId);
 }
 void UpdateDesk(){
