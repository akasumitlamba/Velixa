...
public class MainForm:Form {
 ...
 public MainForm(string[] argv){args=argv;string role=Wire.LoadSecret("role.bin","");Text="Velixa";BackColor=Bg;ForeColor=Color.White;Font=Visual.Font(14);AutoScaleMode=AutoScaleMode.None;DoubleBuffered=true;ClientSize=new Size(1200,850);MinimumSize=new Size(920,690);StartPosition=FormStartPosition.CenterScreen;string icon=Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"velixa.ico");if(File.Exists(icon))Icon=new Icon(icon);
  var brand=new Panel{Dock=DockStyle.Top,Height=76};Controls.Add(brand);var logo=new PictureBox{Location=new Point(36,22),Size=new Size(32,32),SizeMode=PictureBoxSizeMode.Zoom};string png=Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"velixa-logo.png");if(File.Exists(png))logo.Image=Image.FromFile(png);brand.Controls.Add(logo);LabelAt(brand,"Velixa",80,23,200,30,19,Color.White,true);var settings=Button("Settings",Settings);settings.Name="Settings";settings.SetBounds(920,18,152,40);settings.Anchor=AnchorStyles.Top|AnchorStyles.Right;brand.Controls.Add(settings);brand.Resize+=(s,e)=>settings.Left=brand.Width-208;settings.Left=brand.Width-208;
 ...
 void Attach(){
  ...
  microphone.Status=message=>{tray.Text=(message=="Microphone sharing is on"||message=="Receiving shared microphone")?"Velixa · Microphone sharing":"Velixa";UpdateDesk();status.Text=message;};
 ...
 void DeskView(){
  Clear();int savedMic;int.TryParse(Wire.LoadSecret("mic-device.bin","-1"),out savedMic);activeMicId=savedMic;activeMicPeer=Wire.LoadSecret("mic-peer.bin","");activeMicName=Wire.LoadSecret("mic-name.bin","");
  var layout=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=1,RowCount=5,Margin=Padding.Empty,Padding=Padding.Empty};layout.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));foreach(float height in new[]{56f,64f,64f})layout.RowStyles.Add(new RowStyle(SizeType.Absolute,height));layout.RowStyles.Add(new RowStyle(SizeType.Percent,100));layout.RowStyles.Add(new RowStyle(SizeType.Absolute,52));content.Controls.Add(layout);
  var heading=new Panel{Dock=DockStyle.Fill,Margin=Padding.Empty};layout.Controls.Add(heading,0,0);deskButton=Button(session.DeskName+"  ▾",DesksMenu);deskButton.SetBounds(20,0,260,40);heading.Controls.Add(deskButton);var add=Button("+  Add device",PairDialog,true);add.Name="AddDevice";heading.Controls.Add(add);AlignAction(heading,add,0);add.Visible=network!=null&&network.IsHost;
  var source=new SoftPanel{Dock=DockStyle.Fill,Margin=new Padding(0,0,0,8)};layout.Controls.Add(source,0,1);LabelAt(source,"Keyboard & mouse",20,7,300,18,12,Muted);sourceName=LabelAt(source,"",20,27,480,23,15,Color.White,true);sourceName.AutoEllipsis=true;sourceButton=Button("Change  ▾",SourceMenu);source.Controls.Add(sourceButton);AlignAction(source,sourceButton,8);source.Resize+=(sender,e)=>sourceName.Width=Math.Max(100,source.Width-212);
  var micRow=new SoftPanel{Dock=DockStyle.Fill,Margin=new Padding(0,0,0,8)};layout.Controls.Add(micRow,0,2);LabelAt(micRow,"Microphone",20,7,300,18,12,Muted);var micLabel=LabelAt(micRow,MicName(),20,27,480,23,15,Color.White,true);micLabel.AutoEllipsis=true;micButton=Button("Choose  ▾",()=>MicMenu(micLabel));micRow.Controls.Add(micButton);AlignAction(micRow,micButton,8);micRow.Resize+=(sender,e)=>micLabel.Width=Math.Max(100,micRow.Width-212);
  ...
  var bottom=new Panel{Dock=DockStyle.Fill,Margin=Padding.Empty};layout.Controls.Add(bottom,0,4);deskCount=LabelAt(bottom,"",20,17,600,22,12,Muted);pauseButton=Button("Pause sharing",()=>{paused=!paused;if(paused)session.Input.Home();session.Input.Enabled=!paused&&session.Connected&&session.Source==Network.LocalId;UpdateDesk();});bottom.Controls.Add(pauseButton);AlignAction(bottom,pauseButton,8);bottom.Resize+=(sender,e)=>deskCount.Width=Math.Max(100,bottom.Width-212);UpdateDesk();
 }
...
public class DeskCanvas:Control {
 ...
 public DeskCanvas(DeskSession s){
  ...
  var fit=MainForm.Button("Fit",()=>{zoom=1;Invalidate();});var minus=MainForm.Button("−",()=>{zoom=Math.Max(.5f,zoom-.2f);Invalidate();});var plus=MainForm.Button("+",()=>{zoom=Math.Min(2f,zoom+.2f);Invalidate();});Controls.AddRange(new Control[]{fit,minus,plus});Action place=()=>{fit.SetBounds(Width-172,10,60,32);minus.SetBounds(Width-106,10,38,32);plus.SetBounds(Width-62,10,42,32);};Resize+=(sender,e)=>place();place();
 ...
 void Transform(){var d=session.Devices;if(d.Count==0){scale=1;ox=40;oy=60;return;}float left=d.Min(v=>v.Bounds.Left),right=d.Max(v=>v.Bounds.Right),top=d.Min(v=>v.Bounds.Top),bottom=d.Max(v=>v.Bounds.Bottom);float contentW=right-left+60,contentH=bottom-top+60;float availW=Math.Max(100,Width-60),availH=Math.Max(100,Height-LegendHeight-75);scale=Math.Min(2.4f,Math.Min(availW/contentW,availH/contentH))*zoom;ox=Width/2f-(left+right)/2*scale;oy=50+(availH)/2f-(top+bottom)/2*scale;}
 public RectangleF Card(Device d){var r=d.Bounds;return new RectangleF(ox+r.X*scale,oy+r.Y*scale,r.Width*scale,r.Height*scale);}
  protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);if(!Capture)Transform();var g=e.Graphics;g.SmoothingMode=SmoothingMode.AntiAlias;g.Clear(MainForm.Bg);Visual.Fill(g,new RectangleF(0,0,Width,Height),20,MainForm.PanelColor);
 Visual.Text(g,Selected==null?"Arrange your screens":Selected.name+" · "+(Selected.sleeping?"Sleeping":Selected.Available?"Ready":"Unavailable"),new RectangleF(24,15,Width-250,30),13,MainForm.Muted,true);
 int number=0;foreach(var d in session.Devices){number++;var outer=Card(d);var r=outer;r.Inflate(-Math.Min(8,outer.Width/6),-Math.Min(8,outer.Height/6));if(r.Width<=0||r.Height<=0)continue;var saved=g.Save();g.SetClip(outer);
 ...
 Visual.Text(g,dropHover==null?"Drag to arrange · Click for options":"Drop files to send to "+dropHover.name,new RectangleF(24,Height-36,Width-48,28),12,MainForm.Muted,false,StringAlignment.Center);
 }
 int LegendHeight{get{return ((session.Devices.Count+2)/3)*30+46;}}
 RectangleF Legend(int i){float cell=(Width-48)/3f;return new RectangleF(24+(i%3)*cell,Height-LegendHeight+(i/3)*30,cell-12,28);}
