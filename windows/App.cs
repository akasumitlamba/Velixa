...
public class DeskCanvas:Control {
 ...
 protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);if(!Capture)Transform();var g=e.Graphics;g.SmoothingMode=SmoothingMode.AntiAlias;g.Clear(MainForm.Bg);Visual.Fill(g,new RectangleF(0,0,Width,Height),20,MainForm.PanelColor);Visual.Stroke(g,new RectangleF(.5f,.5f,Width-1,Height-1),20,Color.FromArgb(43,47,61));Visual.Text(g,Selected==null?"Arrange your screens":Selected.name+" · "+(Selected.sleeping?"Sleeping":Selected.Available?"Ready":"Unavailable")+" · "+(int)(Selected.scale*100)+"%",new RectangleF(24,15,Width-48,30),13,Color.FromArgb(190,195,211),true);
  foreach(var d in session.Devices){var r=Card(d);bool source=d.id==session.Source,phone=d.kind=="Android",online=d.Available;Color border=!online?Color.FromArgb(62,66,80):source?Color.FromArgb(157,134,251):Color.FromArgb(107,113,138);float radius=phone?Math.Min(16,r.Width*.15f):8;
   if(d==dropHover){var glow=r;glow.Inflate(8,8);Visual.Fill(g,glow,radius+8,Color.FromArgb(60,141,119,249));Visual.Stroke(g,glow,radius+8,Color.FromArgb(180,151,255),2.5f);}
   else if(d==Selected){var halo=r;halo.Inflate(7,7);Visual.Stroke(g,halo,radius+7,Color.FromArgb(85,73,117),1);}
   ...
   float textSize=Math.Max(9,Math.Min(14,r.Width/10));Visual.Text(g,d.name,new RectangleF(r.X+8,r.Y+r.Height/2-(r.Height>=70?23:12),r.Width-16,24),textSize,online?Color.White:MainForm.Muted,true,StringAlignment.Center);if(r.Height>=70)Visual.Text(g,d.sleeping?"Sleeping":!d.online?"Offline":!d.awake?"Screen asleep":source?"Input source":"Ready",new RectangleF(r.X+6,r.Y+r.Height/2+1,r.Width-12,21),Math.Max(9,textSize-2),MainForm.Muted,false,StringAlignment.Center);
   foreach(var neighbor in session.Devices.Where(v=>v!=d&&v.Available)){foreach(string side in new[]{"left","right","top","bottom"}){double from,to;if(!d.Available||!DeskGeometry.Segment(d.Bounds,neighbor.Bounds,side,out from,out to))continue;using(var pen=new Pen(DeviceColor(neighbor.id),3)){float lo=(float)from,hi=(float)to;if(side=="left"||side=="right"){float x=side=="left"?r.Left+2:r.Right-2;g.DrawLine(pen,x,r.Top+r.Height*lo+2,x,r.Top+r.Height*hi-2);}else{float y=side=="top"?r.Top+2:r.Bottom-2;g.DrawLine(pen,r.Left+r.Width*lo+2,y,r.Left+r.Width*hi-2,y);}}}}
  }
  string hint=dropHover!=null?"Drop files to send to "+dropHover.name:(session.Devices.Count<=1?"Add your other devices to bring your desk together.":"Drag screens to arrange · Click for options · Drop files onto a Windows device to send");
  Visual.Text(g,hint,new RectangleF(24,Height-43,Width-48,28),12,dropHover!=null?Color.FromArgb(200,185,255):MainForm.Muted,false,StringAlignment.Center);
 }
