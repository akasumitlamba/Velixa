...
public partial class MainForm:Form {
 public static Color Bg=Color.FromArgb(13,18,27),PanelColor=Color.FromArgb(19,26,37),Muted=Color.FromArgb(155,166,189),Accent=Color.FromArgb(116,97,255);
 ...
 void Clear(){if(discovery!=null){discovery.Stop();discovery.Dispose();discovery=null;}foreach(Control child in content.Controls.Cast<Control>().ToArray())child.Dispose();}
 ...
 internal void ForgetDevice(Device device){if(device==null||device.id==Network.LocalId)return;bool coordinator=network!=null&&!network.IsHost&&device.id==Wire.LoadSecret("remote-id-v2.bin","");string message=coordinator?"Leave this desk and forget its pairing on this PC?":session.Connected?"Remove "+device.name+" from saved desks? Pair again to reconnect.":"Remove "+device.name+" here now? Removal will sync when the coordinator reconnects.";if(!ConfirmAction("Forget device?",message,"Forget"))return;if(coordinator){LeaveDesk();return;}session.RemoveDevice(device.id);}
 void LeaveDesk(){Stop();foreach(var key in new[]{"role.bin","remote-token-v2.bin","remote-fp-v2.bin","remote-id-v2.bin","host.bin","client-devices.bin","pending-removals.bin"})Wire.SaveSecret(key,"");paused=false;Welcome();}
 void RetryDesk(){if(network!=null&&network.IsHost)return;paused=false;StartReceiver(Wire.LoadSecret("host.bin",""),"");}
 void Ui(Action a){if(IsDisposed||!IsHandleCreated)return;try{BeginInvoke(a);}catch{}}
 void Attach(){
  session.Changed=()=>UpdateDesk();session.Input.Notify=message=>{status.Text=message;};var current=session;
  ...
  microphone=new MicrophoneShare((to,body)=>{if(session==current)current.SendShared(to,body);},Ui,id=>current.Connected&&current.Devices.Any(d=>d.id==id&&d.online&&!d.sleeping&&d.sharing));
  microphone.ReceiveEnabled=true;
  microphone.AllowRemoteRequests=Wire.LoadSecret("mic-allow-requests.bin","true")=="true";
  microphone.OutputName=Wire.LoadSecret("mic-output-name.bin","");
  microphone.MicsChanged=()=>{if(micButton!=null&&!micButton.IsDisposed)micButton.Invalidate();};
  microphone.Targets=()=>current.Connected?current.Devices.Where(d=>d.id!=Network.LocalId&&d.Available&&d.sharing&&d.kind=="Windows").Select(d=>d.id).ToArray():new string[0];
  microphone.DiscoveryTargets=()=>current.Connected?current.Devices.Where(d=>d.id!=Network.LocalId&&d.Available&&d.sharing).Select(d=>d.id).ToArray():new string[0];
  sharing.AudioMessage=microphone.Handle;
  microphone.Status=message=>{if(micLabel!=null&&!micLabel.IsDisposed)micLabel.Text=message;tray.Text=(microphone.Sending||microphone.Receiving)?"Velixa · Microphone sharing":"Velixa";UpdateDesk();status.Text=message;};
 ...
 void MicMenu(Label label){
  ...
  bool selected=microphone!=null&&(microphone.Receiving||microphone.Pending);
  var off=new ToolStripMenuItem("Microphone off"){Checked=!selected,Padding=new Padding(12,7,12,7)};
  off.Click+=(s,e)=>{
   ...
   if(microphone!=null)microphone.StopReceiving();
   label.Text=MicName();
  };
  menu.Items.Add(off);
  menu.Items.Add(new ToolStripSeparator());
  var localMics=WaveAudio.FullDevices(true);
  bool canSend=microphone!=null;
  ...
  foreach(var m in localMics){
   ...
   var item=new ToolStripMenuItem(displayName){Checked=selected&&(activeMicPeer==""||activeMicPeer==Network.LocalId)&&activeMicId==id,Padding=new Padding(12,7,12,7)};
   item.Enabled=canSend;item.Click+=(s,e)=>{
    if(!microphone.SelectLocal(id)){label.Text=microphone.State;MicrophoneDialog();return;}
    ...
    label.Text=microphone.State;
   };
   menu.Items.Add(item);
  }
  if(microphone!=null&&session!=null){
   foreach(var kv in microphone.RemoteMics.Where(k=>session.Devices.Any(d=>d.id==k.Key&&d.Available))){
    ...
    foreach(var rm in kv.Value){
     ...
     var rItem=new ToolStripMenuItem(rDisplay){Checked=selected&&activeMicPeer==peerId&&activeMicId==rDevId,Padding=new Padding(12,7,12,7)};
     rItem.Click+=(s,e)=>{
      if(!microphone.RequestRemoteMic(peerId,rDevId,rName)){label.Text=microphone.State;MicrophoneDialog();return;}
  ...
  menu.Items.Add(new ToolStripSeparator());menu.Items.Add("Microphone output…",null,(s,e)=>MicrophoneDialog()).Enabled=microphone!=null;menu.Show(micButton,0,micButton.Height+6);
 }
 void UpdateDesk(){
  if(desk==null||desk.IsDisposed||session==null||sourceName==null||sourceName.IsDisposed)return;
  if(deskButton!=null&&!deskButton.IsDisposed)deskButton.Text="Desks";
  ...
  deskCount.Text=session.Connected?online+" device"+(online==1?"":"s")+" connected":"Waiting for your desk to come back online";
  pauseButton.Text=paused?"Resume input":"Pause input";
  pauseButton.Visible=true;
  status.Text=session.Input.Active==null?"Ctrl + Alt + Backspace to return":"You’re controlling "+session.Input.Active.Name+".  Ctrl + Alt + Backspace brings you back.";
  RefreshInspector();RefreshDevicePage();
  if(paused)session.Input.Enabled=false;

 }
 void Settings(){using(var dialog=new Form{AutoScaleMode=AutoScaleMode.None,Text="Settings",ClientSize=new Size(480,440),StartPosition=FormStartPosition.CenterParent,FormBorderStyle=FormBorderStyle.FixedDialog,MaximizeBox=false,MinimizeBox=false,BackColor=Bg,ForeColor=Color.White,Font=Font}){Visual.DarkTitle(dialog);LabelAt(dialog,"Make yourself at home",24,22,432,36,24,Color.White,true);LabelAt(dialog,"Minimize or close to keep sharing from the tray.",24,67,432,30,13,Muted);var clip=new CheckBox{Text="Share text and image clipboard",Checked=sharing!=null&&sharing.ClipboardEnabled,Enabled=sharing!=null,AutoSize=false,Location=new Point(26,111),Size=new Size(424,30),ForeColor=Color.White};clip.CheckedChanged+=(s,e)=>{sharing.ClipboardEnabled=clip.Checked;Wire.SaveSecret("clipboard-enabled.bin",clip.Checked?"true":"false");};dialog.Controls.Add(clip);var files=new CheckBox{Text="Receive dropped files in Downloads / Velixa",Checked=sharing!=null&&sharing.FilesEnabled,Enabled=sharing!=null,AutoSize=false,Location=new Point(26,148),Size=new Size(424,30),ForeColor=Color.White};files.CheckedChanged+=(s,e)=>{sharing.FilesEnabled=files.Checked;Wire.SaveSecret("files-enabled.bin",files.Checked?"true":"false");};dialog.Controls.Add(files);var mic=Button("Microphone output…",MicrophoneDialog);mic.Enabled=microphone!=null;mic.SetBounds(24,198,432,44);dialog.Controls.Add(mic);var lights=Button("Preview edge lights",()=>{if(session!=null)session.Input.PreviewEdges();});lights.SetBounds(24,256,432,44);dialog.Controls.Add(lights);var leave=Button("Leave this desk",()=>{dialog.Close();Stop();Wire.SaveSecret("role.bin","");Welcome();});leave.SetBounds(24,314,432,44);leave.Enabled=session!=null;dialog.Controls.Add(leave);var done=Button("Done",()=>dialog.Close(),true);done.SetBounds(328,382,128,40);dialog.Controls.Add(done);dialog.ShowDialog(this);}}
 void MicrophoneDialog(){
  if(microphone==null)return;
  using(var f=new Form{AutoScaleMode=AutoScaleMode.None,Text="Microphone output",ClientSize=new Size(570,430),StartPosition=FormStartPosition.CenterParent,FormBorderStyle=FormBorderStyle.FixedDialog,MaximizeBox=false,MinimizeBox=false,BackColor=Bg,ForeColor=Color.White,Font=Font}){
   Visual.DarkTitle(f);LabelAt(f,"One microphone selector for this PC",24,20,522,36,23,Color.White,true);
   LabelAt(f,"Choose any paired PC’s microphone from My Desk. Its source starts automatically while you use it.",24,68,522,48,14,Muted);
   LabelAt(f,"Microphone output for your apps",24,128,522,26,16,Color.White,true);
   var outputs=new DarkComboBox{Location=new Point(24,166),Width=522};outputs.Items.Add("Automatic · virtual audio cable");
   var devices=WaveAudio.FullDevices(false);outputs.Items.AddRange(devices);outputs.SelectedIndex=0;
   for(int i=0;i<devices.Length;i++)if(devices[i].Name==microphone.OutputName)outputs.SelectedIndex=i+1;
   outputs.SelectedIndexChanged+=(s,e)=>{microphone.StopReceiving();microphone.OutputDevice=-1;microphone.OutputName=outputs.SelectedIndex==0?"":((WaveAudio.Device)outputs.SelectedItem).Name;Wire.SaveSecret("mic-output-name.bin",microphone.OutputName);};f.Controls.Add(outputs);
   LabelAt(f,"Install VB-CABLE once on this PC, then choose CABLE Output in your calling or recording app. Automatic routing uses CABLE Input. Selecting speakers here plays the microphone aloud.",24,214,522,70,14,Muted);
   var guide=new LinkLabel{Text="Get VB-CABLE",Location=new Point(24,292),Size=new Size(180,24),LinkColor=Accent};guide.LinkClicked+=(s,e)=>System.Diagnostics.Process.Start("https://vb-audio.com/Cable/");f.Controls.Add(guide);
   var allow=new CheckBox{Text="Let paired PCs use this PC’s microphones",Checked=microphone.AllowRemoteRequests,Location=new Point(24,330),Size=new Size(522,28)};
   allow.CheckedChanged+=(s,e)=>{microphone.AllowRemoteRequests=allow.Checked;Wire.SaveSecret("mic-allow-requests.bin",allow.Checked?"true":"false");if(!allow.Checked)microphone.StopSending();};f.Controls.Add(allow);
   var done=Button("Done",()=>f.Close(),true);done.SetBounds(426,376,120,38);f.Controls.Add(done);f.CancelButton=done;f.ShowDialog(this);
  }
 }
...
public class DeskCanvas:Control {
 ...
 void Transform(){var d=session.Devices;if(d.Count==0){scale=1;ox=40;oy=60;return;}float left=d.Min(v=>v.Bounds.Left),right=d.Max(v=>v.Bounds.Right),top=d.Min(v=>v.Bounds.Top),bottom=d.Max(v=>v.Bounds.Bottom);float contentW=right-left+60,contentH=bottom-top+60;float availW=Math.Max(100,Width-40),availH=Math.Max(100,Height-40);scale=Math.Min(2.4f,Math.Min(availW/contentW,availH/contentH))*zoom;ox=Width/2f-(left+right)/2*scale;oy=20+(availH)/2f-(top+bottom)/2*scale;}
 public RectangleF Card(Device d){var r=d.Bounds;return new RectangleF(ox+r.X*scale,oy+r.Y*scale,r.Width*scale,r.Height*scale);}
  protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);if(!Capture)Transform();DeskArtwork.Paint(e.Graphics,ClientRectangle,session,Selected,dropHover,Card);}
 int LegendHeight{get{return 0;}}
 RectangleF Legend(int i){return RectangleF.Empty;}
 public void Fit(){zoom=1;Invalidate();}
 public void Zoom(float step){zoom=Math.Max(.5f,Math.Min(2f,zoom+step));Invalidate();}
 protected override void OnMouseWheel(MouseEventArgs e){base.OnMouseWheel(e);Zoom(e.Delta>0?.1f:-.1f);}
 public void EditSize(Device device){if(device!=null&&session.Devices.Contains(device))SizeDialog(device);}
 public void ShowSelectedOptions(){if(Selected!=null)Options(new Point(Math.Max(0,Width-220),40));}
 ...
 protected override void OnMouseUp(MouseEventArgs e){base.OnMouseUp(e);Capture=false;if(Selected!=null){if(moved){double x=Selected.x,y=Selected.y;Selected.x=originalX;Selected.y=originalY;session.Position(Selected.id,x,y);}else if(e.Button==MouseButtons.Right||(e.X>Card(Selected).Right-44&&e.Y<Card(Selected).Top+48))Options(e.Location);}Invalidate();}
 protected override void Dispose(bool disposing){if(disposing)deviceTip.Dispose();base.Dispose(disposing);}
 public static Color DeviceColor(string id){return id==Network.LocalId?Color.FromArgb(236,190,75):Color.FromArgb(87,161,240);}

 void Options(Point point){var d=Selected;if(d==null)return;var menu=DarkMenu.Create();menu.Items.Add(d.name+" · "+(d.Available?"Connected":d.sleeping?"Excluded":"Offline")).Enabled=false;menu.Items.Add(d.sleeping?"Include in input sharing":"Exclude from input sharing",null,(s,e)=>session.SleepDevice(d.id,!d.sleeping));menu.Items.Add("Adjust screen size…",null,(s,e)=>SizeDialog(d));var use=menu.Items.Add("Use this keyboard and mouse",null,(s,e)=>session.UseSource(d.id));use.Enabled=d.Available&&d.kind=="Windows";if(d.id!=Network.LocalId){menu.Items.Add(new ToolStripSeparator());var remove=menu.Items.Add("Forget device…",null,(s,e)=>{((MainForm)FindForm()).ForgetDevice(d);});remove.Enabled=true;}menu.Show(this,point);}
