...
public partial class MainForm {
 ...
 var sleep=Button(selected.sleeping?"Include in sharing":"Exclude from sharing",()=>{session.SleepDevice(selected.id,!selected.sleeping);ShowSection("Devices");});sleep.SetBounds(170,144,180,36);sleep.Enabled=session.Connected;row.Controls.Add(sleep);
 if(d.id!=Network.LocalId){var forget=Button("Forget device",()=>{if(ConfirmAction("Forget device?","Remove "+selected.name+" from every saved desk? You will need to pair it again.","Forget device")){session.RemoveDevice(selected.id);ShowSection("Devices");}});forget.SetBounds(360,144,140,36);forget.Enabled=session.Connected;row.Controls.Add(forget);}
 }
 ...
 card("Ctrl + Alt + Backspace","Return control to this PC when your pointer is on another screen. This shortcut remains available while Velixa runs in the system tray.",null,null);
 card("Arrange with your mouse","Drag a device card to match its physical position. Touching screen edges allow the pointer to move between devices.",()=>ShowSection("My Desk"),"Arrange screens");
 card("Select and inspect","Click a device to see its settings. Right-click a card or use its menu for screen size, sleep and removal options.",null,null);
 card("Keyboard navigation","Use Tab to move between controls, Enter to activate buttons, and Space to change sharing switches.",null,null);
 }else if(name=="About"){
 card("Velixa · "+Application.ProductVersion,"One input. Everywhere.\nShare your keyboard, mouse, clipboard, files and microphone across your local desk.",null,null);
 card("Private by design","Pair devices on your local network. Connections are encrypted, with no account or cloud relay required.",null,null);
 card("Windows and Android","Windows PCs can send input. Android receives input through its accessibility service. Keep both devices connected to the same network.",null,null);
 ...
 card("Microphone setup","Select a source and destination, configure incoming audio, and manage capture permission.",MicrophoneDialog,"Configure microphone");
 ...
 card("Leave this desk","Disconnect this PC and return to setup. You can create or join a desk again.",()=>{if(ConfirmAction("Leave this desk?","Sharing on this PC will stop and you will return to setup.","Leave desk")){Stop();Wire.SaveSecret("role.bin","");Welcome();}},"Leave desk");
