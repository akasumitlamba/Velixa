...
& "$bt/aapt2.exe" link -o build/android-test/base.apk -I $jar --manifest tests/android/ThemeManifest.xml
& javac -encoding UTF-8 -source 8 -target 8 -bootclasspath "$jar;$bt/core-lambda-stubs.jar" -classpath 'build/android/classes.jar;deps/zxing.jar' -d build/android-test/classes tests/android/Theme.java
...
$result = & "$sdk/platform-tools/adb.exe" -s $Serial shell am instrument -w com.velixa.tests/com.velixa.app.Theme
$result
if($LASTEXITCODE -ne 0 -or ($result -join "`n") -notmatch 'PASS Android theme'){throw 'Theme check failed'}
foreach($name in @('theme-setup.png','theme-pair.png','theme-desk.png','theme-bottom.png')){& "$sdk/platform-tools/adb.exe" -s $Serial pull "/sdcard/Android/data/com.velixa.app/files/$name" "build/$name"}
